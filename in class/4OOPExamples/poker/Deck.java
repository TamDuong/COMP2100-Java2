...
public class Deck
{
    ...
    {
        Deck d = new Deck();    //to constructor
        System.out.println(d);  //to toString method. this will print the cards in order
        d.shuffle();
        System.out.println();
        System.out.println(d);  // to toString method. print cards that already shuffle
        
        for (int i = 0; i < 52;  i++) {
            System.out.println("Draw returned: " + d.draw());
        }
        System.out.println(d.draw());   //a deck only have 52 cards so when the for loop break then this line just to tell the user that there are no more card
    }
}
